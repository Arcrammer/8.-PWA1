...
(function(){
    ...
    //player stats
    var playerOne = ["Gaga",20,100]; // Stores the name, taken damage, and health of the first player to an array
    var playerTwo = ["Katy",20,100]; // Stores the name, taken damage, and health of the second player to an array
    ...
    function fight(){ // Declaring a named function known as 'fight()' to be called at the end of this script in order to begin the fight
        alert(playerOne[0] + ":" + playerOne[2] + "	*START*	" + playerTwo[0] + ":" + playerTwo[2]); // This is the first alert which the user will see after 'fight()' is called
        for (var i = 0;i < 10; i++) { // Declaring a for() to loop once for each of the 10 rounds of the fight
        //random formula is - Math.floor(Math.random() * (max - min) + min);
            var minDamage1 = playerOne[1] * .5; // Multiplies the amount the first player may lose during the fight with '.5' then stores the result in the local variable 'minDamage1'
            ...
            var f1 = Math.floor(Math.random()*(playerOne[1]-minDamage1)+minDamage1); // Generates a random outcome which will be taken from the first players health then stores that amount in the variable 'f1'
            ...
            //inflict damage
            playerOne[2] -= f1; // Takes the amount stored in the 'f1' variable from the first players' health amount
            playerTwo[2] -= f2; // Takes the amount stored in the 'f2' variable from the second players' health amount

            console.log(playerOne[0] + ": " + playerOne[2] + "\n" + playerTwo[0] + ":" + playerTwo[2]); // Outputs the current players names and health amounts to the console
            ...
            if (result === "no winner") // Either alert()'s the user of the current players' health and the current round or alert()'s the user of the result of the fight
            {
                ...
                alert(playerOne[0]+":"+playerOne[2]+" *ROUND "+round+" OVER"+"*  "+playerTwo[0]+":"+playerTwo[2]); // Alert()'s the user of the current round and health amounts of both players
    ...
    function winnerCheck(){ // Declared a function named 'winnerCheck()' which will check to see which player has won the fight after fight() has executed
        ...
        if (playerOne[2]<1 && playerTwo[2]<1) // if() to be executed when the first and second players both have health less than the amount of 1
        ...
        } else if(playerOne[2]<1){ // Else() to execute if the first player has health lower than the amount of 1
            result =playerTwo[0]+" WINS!!!" // Reassigns the value of the 'result' variable if the first player has health lower than 1 to declare that the second player has won the fight
        } else if (playerTwo[2]<1)// Else() to execute if the second player has health lower than the amount of 1
        {
            result = playerOne[0]+" WINS!!!" // Reassigns the value of the 'result' variable if the second player has health lower than 1 to declare that the second player has won the fight
            if(playerOne[0].toLowerCase() == "gaga") {
