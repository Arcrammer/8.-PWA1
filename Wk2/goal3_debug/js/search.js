...
(function(){ // Self-executing function to execute when this document is loaded
	...
	var validate = function(query) { // Function to be called upon query submission to trim the query of spaces and check its' length
		...
		if(query.length < 3) { // If() to execute if the length of the (now trimmed) query is less than 3 characters long
			...
			return; // Empty return for the if() at line 21
		}; // If() 21
	...
	var search = function(query) {
		...
		for(var i = 0, j = db.length; i < j; i++) { // For() to execute once for each database item
			...
			var dbitem = db[i].toLowerCase().substring(0, dbTitleEnd); // Assigns the values of each database item to 'dbitem'
			...
			for(var ii=0, jj=queryArray.length; ii<jj; ii++) { // For() to loop once for each word in 'queryArray' defined on line 36
				...
				if(compare !== -1) { // If() to execute if 'compare' exists (meaning a matching word was found)
					results.push(db[i]); // Adds the matching result to the 'results' array for later output
				}; // If() 57
			}; // For() 51
		}; // For() 42
		
		results.sort(); // Sorts the items compiled in the 'results' array alphabetically ascending
		...
		if(results.length = 0) { // If() to execute if no results were pushed to the 'results' array
			...
			showMatches(results); // Calls 'showMatches()' if the array 'results' contains items
		}; // If() 66
	}; // Search() 33
	...
	var showMatches = function(results) { // Declaring a function under the variable 'showMatches'
		...
		for(var i=0, j=results.length; i<j; i++) { // For() to execute once for every item in the array 'results'

			// title of video ends with pipe
			// pull the title's string using index numbers
			titleEnd = results[i].indexOf('|'); // Defining a variable to hold the position of the vertical bar character within the title of the returned item as an integer value
			...
			url = results[i].substring(results[i].indexOf('|')+1, results[i].length); // Defining the variable 'URL' to hold the returned link to the item from the database
